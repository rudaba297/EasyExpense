...
function initSignupForm() {
  ...
  form.addEventListener("submit", async function (e) {
    // If the form posts to a PHP endpoint, allow native submission (server-side flow)
    const action = form.getAttribute("action") || "";
    if (action.toLowerCase().endsWith(".php")) {
      return; // let the browser perform a normal POST to the PHP handler
    }
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/filesignup`, {
        method: "POST",
        headers: {
...
function initSigninForm() {
  ...
  // Form submission
  form.addEventListener("submit", async function (e) {
    // If the form posts to a PHP endpoint, allow the native submission (server-side flow)
    const action = form.getAttribute("action") || "";
    if (action.toLowerCase().endsWith(".php")) {
      return; // let the browser perform a normal POST
    }
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/filelogin`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
